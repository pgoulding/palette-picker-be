...
describe('Server', () => {
  ...
  describe('GET /palettes', () => {
    ...
    it('HAPPY hould return a 200 and all of the palettes', async () => {
  ...
  describe('PATCH /projects', () => {
    it('HAPPY it should return a 202 when patching a project', async () => {
      ...
      const newName = await database('projects')
        .where('id', 1)
        .select()
      expect(res.text).toEqual('Project ID# 1 has been updated')
    ...
    it('SAD should return a 404 if no project with matching id is found', async () => {
      ...
      const res = await request(app)
        ...
        .send(namePatch)
      expect(res.status).toBe(404)
      expect(res.text).toEqual('Project ID# 100 does not exist.')
    });
  });
  describe('PATCH /palettes', () => {
    it('HAPPY it should return a 202 when patching a palette', async () => {
      const palettePatch = { 
        name: 'PATCHED',
        color_3: 'color3'
      }
      ...
      expect(res.text).toEqual('Palette ID# 1 has been updated')
    ...
    it('SAD should return a 404 if no palette with matching id is found', async () => {
      ...
      const res = await request(app)
        ...
        .send(palettePatch)
      expect(res.status).toBe(404)
      expect(res.text).toEqual('Palette ID# 100 does not exist.')
    });
  });
  ...
  describe('DELETE /api/v1/palettes', () => {
    ...
    it('HAPPY: Should delete palette if the ID exists', async () => {
      ...
      expect(res.status).toEqual(202)
      expect(res.text).toEqual('Palette ID# 1 has been deleted.')
    })
    it('SAD: Should send an error if the id doesnt exist', async () => {
      const res = await request(app)
      .delete('/api/v1/palettes/112')
    expect(res.status).toEqual(404)
    expect(res.text).toEqual('Palette ID# 112 does not exist.')
    })
  })
});
